...
void animationManager::Initialize()
{
  ...
  if (this->AvailAnimations > 0 && this->Interactor)
  {
    ...
    // b this->Interactor->SetInteractorOn(this->ProgressWidget);
  ...
  */
  ...
  {
      qDebug() << "No animation available";
    return;
  }
  else
  {
    qDebug() << "Animation(s) available are:";
  }
...
bool animationManager::LoadAtTime(double timeValue)
{
  ...
  if (this->AvailAnimations == 0)
  {
    qDebug() << "No animation available, cannot load a specific animation time";
    this->Playing = false;
    return false;
  }
  ...
  if (timeValue < this->TimeRange[0])
  {
    ...
    {
      qDebug() << "Animation time " << timeValue << " is outside of range [" <<  this->TimeRange[0] << ", " <<
  ...
  {
    qDebug() << "Could not load time value: " << this->CurrentTime;
    return false;
  }
#else
...
void animationManager::CycleAnimation()
{
  ...
  {
    qDebug() << "scene.animation.index is deprecated, please use " << "scene.animation.indices instead";
...
void animationManager::PrepareForAnimationIndices()
{
  ...
  if (std::any_of(animIndices.begin(), animIndices.end(), [](int idx) { return idx < 0; }))
  {
    ...
    {
      qDebug() << "Multiple animation indices have been specified include a negative one, all " << "animations will be selected";
    }
  ...
  if (!animIndices.empty() && animIndices != std::vector<int>{ 0 })
  {
    if (this->AvailAnimations == 0)
    {
        qDebug() << "Animation indices have been specified but there are no animation available in this file.";
    }
    else
    {
#if VTK_VERSION_NUMBER >= VTK_VERSION_CHECK(9, 4, 20250507)
      switch (this->Importer->GetAnimationSupportLevel())
      {
        case vtkImporter::AnimationSupportLevel::UNIQUE:
          ...
          {
            qDebug() << "Non-zero or multiple animation indices have been specified but currently " << "loaded file does not support it.";
          }
          break;
        case vtkImporter::AnimationSupportLevel::SINGLE:
          if (this->Options.scene.animation.indices.size() > 1)
          {
              qDebug() << "Multiple animation indices have been specified but currently loaded files may " << "not support enabling multiple animations";
          }
          break;
        default:
          // NONE is unreachable
  ...
  for (int idx : this->PreparedAnimationIndices.value())
  {
    if (idx >= this->AvailAnimations)
    {
      qDebug() << "Specified animation index: " << idx << " is not in range [0, " << this->AvailAnimations - 1 << "], ignoring";
    }
    this->Importer->EnableAnimation(idx);
  }

  // Display currently selected animation
  qDebug() << "Current animation is: " << this->GetAnimationName();
  ...
  if (foundAnimation)
  {
    ...
    {
      qDebug() << "Animation(s) time range delta is invalid: [" << this->TimeRange[0] << ", " << this->TimeRange[1] << "]. Swapping range.";
      std::swap(this->TimeRange[0], this->TimeRange[1]);
    }
    qDebug() << "Current animation time range is: [" << this->TimeRange[0] << ", " << this->TimeRange[1] << "].";
  }
}
